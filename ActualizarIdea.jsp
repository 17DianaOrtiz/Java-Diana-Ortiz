...
<div class="register-container">
    <div class="register-header">
        <h2>Actualizar Idea de grado</h2>
        <p>Complete el formulario</p>
    </div>
    <div class="form-section">

        <form action="CargarIdea.jsp" method="get">
            <label class="mt-2" for="id">Ingrese el ID de la idea:</label>
            <input type="text" name="id" id="id" required>
            <button class="btn-warning" type="submit">Cargar Datos</button>
        </form>

        
        <form action="ActIdea.jsp" method="post">
            ...
            <div class="form-group">
                <label for="idea"><i class="fas fa-user-tag"></i> Coordinador</label>
                <select class="form-control" id="idea" name="idea" required>
                    <option disabled>Seleccione un coordinador</option>
                    <c:forEach var="idea" items="${listaideas.rows}">
                        <option value="${idea.id}" ${idea.nombre == param.nombre ? 'selected' : ''}>
                            ${idea.nombre}
                        </option>
                    </c:forEach>
                </select>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="titulo" value="${param.titulo}" required>
            </div>
            <div class="form-group">
                <label for="descripcion">Descripción:</label>
                <textarea class="form-control" name="descripcion"  required>${param.descripcion}</textarea>
            </div>
            <div class="form-group">
                <label for="area" >Area:</label>
                <input class="form-control" name="area" value="${param.area}" required>
            </div>
            <div class="form-group">
                <label for="enlace">Enlace al archivo PDF:</label>
                <input type="text" class="form-control" name="enlace" value="${param.enlace}"  required>
            </div>
            <button type="submit" class="btn btn-success btn-custom mt-1">Actualizar Idea</button>
            <a href="VisualizarIdeas.jsp" class="btn btn-warning btn-custom mt-1">Ver Ideas</a>
            <a href="Ideas.jsp" class="btn btn-light btn-custom mt-1">Regresar</a>
